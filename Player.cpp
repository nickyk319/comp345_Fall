...
void Player::issueOrder() {
    ...
    while (reinforcements > 0) {
        ...
        cout << "\nHow many reinformcements would you like to deploy to: "<< Map::worldMap.territories[tID-1]->getTName()<<"?";
    ...
    for (auto i : todefendlist) {
        ...
        for (auto j : territories) {
            if (j->tID == tID) {
                ...
                cin >> num;
                Advance* a = new Advance(this, Map::worldMap.territories[i-1]->getTID(), Map::worldMap.territories[tID-1]->getTID(), num);
    ...
    while (!flag) {
        ...
        case airlift: {
            ...
            int tID2;
            cin >> tID2;
            Airlift* a = new Airlift(this, Map::worldMap.territories[tID2 - 1]->getTID(), Map::worldMap.territories[tID - 1]->getTID(), num);
            OrderList.push_back(*a);
            flag = true;
            break;
        }
...
void Player::printAdjacentTerritories() {
    ...
    for (int i = 0; i < Map::worldMap.territories.size(); i++) {
        ...
        if (!territoryPlayer._Equal(playerName)) {
