...
int main() {
	...
	while (true) {
		...
		else if (command == "issueorder" && (ge->getState() == GameState::assign_reinforcement || ge->getState() == GameState::issue_orders)) {
			ge->issueOrder();
		}
		...
		else if (command == "testgameloop") {
			cout << "now test main game loop" << endl;
			ge->mainGameLoop();
		}
		else {
